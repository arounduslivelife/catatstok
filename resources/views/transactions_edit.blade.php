...
<div class="top-header" style="background: {{ $type == 'IN' ? 'var(--success)' : 'var(--danger)' }}; padding-bottom: 24px;">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 4px;">
        <a href="{{ url('/reports') }}" style="color: white; display: flex; align-items: center;">
            <i class="material-symbols-rounded">arrow_back</i>
        </a>
        <h2 style="margin: 0; color: white;">Edit Barang {{ $type == 'IN' ? 'Masuk' : 'Keluar' }}</h2>
    </div>
    <p style="opacity: 0.9; font-size: 14px; color: white; margin-left: 36px; margin-bottom: 0;">Ubah rincian transaksi</p>
</div>
...
<div class="card" style="margin: 20px; margin-bottom: 80px;">
    <form action="{{ url('/transactions/' . $transaction->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div id="product-list">
            ...
            <div class="product-row" style="display: flex; gap: 12px; margin-bottom: 16px; align-items: flex-end; width: 100%;">
                ...
                <div style="flex: 1; min-width: 0;">
                    ...
                    <input type="number" name="qty[]" class="form-control" min="1" value="{{ $item->qty }}" required placeholder="0" style="height: 50px; border: 2px solid var(--border); border-radius: 12px; text-align: center; font-weight: bold; background-color: #f8f9fa;">
                </div>
                
                <button type="button" class="btn btn-remove" style="background: rgba(255, 59, 48, 0.1); color: var(--danger); border: none; border-radius: 12px; height: 50px; width: 50px; display: {{ count($transaction->items) > 1 || $index > 0 ? 'flex' : 'none' }}; align-items: center; justify-content: center; transition: 0.2s; cursor: pointer;" onclick="this.parentElement.remove()" onmouseover="this.style.background='var(--danger)'; this.style.color='white';" onmouseout="this.style.background='rgba(255, 59, 48, 0.1)'; this.style.color='var(--danger)';">
        ...
        <div class="form-group" style="margin-bottom: 24px;">
            ...
            <textarea name="notes" class="form-control" rows="3" placeholder="Opsional (Misal: Dari supplier X)" style="border: 2px solid var(--border); border-radius: 12px; padding: 12px; background-color: #f8f9fa; transition: 0.3s;" onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor='var(--border)'">{{ $transaction->notes }}</textarea>
        </div>
        
        <div class="form-group" style="margin-bottom: 24px;">
            <label style="font-weight: 600; font-size: 14px; color: var(--text-main); margin-bottom: 6px; display: block;">Bukti Foto</label>
            
            @if($transaction->photo_url)
            <div style="margin-bottom: 12px;">
                <img src="{{ url($transaction->photo_url) }}" style="width: 100%; border-radius: 12px; max-height: 150px; object-fit: cover;">
            </div>
            @endif
            ...
            <div style="position: relative; overflow: hidden; border: 2px dashed var(--border); border-radius: 16px; background: #f8f9fa; padding: 24px 16px; text-align: center; cursor: pointer; transition: all 0.3s;" id="file-drop-area" onmouseover="this.style.borderColor='var(--primary)'; this.style.background='rgba(0,122,255,0.05)';" onmouseout="this.style.borderColor='var(--border)'; this.style.background='#f8f9fa';">
                ...
                <p style="margin: 0; font-weight: 600; font-size: 14px; color: var(--text-main);" id="file-name">{{ $transaction->photo_url ? 'Ketuk untuk Mengganti Foto' : 'Ketuk untuk Mengunggah Foto' }}</p>
        ...
        <button type="submit" class="btn" style="width: 100%; padding: 16px; border-radius: 14px; font-weight: 700; font-size: 16px; background: var(--primary); color: white; box-shadow: 0 4px 12px rgba(0, 122, 255, 0.4); transition: all 0.2s; display: flex; align-items: center; justify-content: center; gap: 8px; border: none;" onmousedown="this.style.transform='scale(0.97)';" onmouseup="this.style.transform='scale(1)';">
            <i class="material-symbols-rounded" style="font-size: 22px;">save</i> Simpan Perubahan
        </button>
    </form>
</div>
